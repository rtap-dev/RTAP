...
public:
        ...
        previewMaxFPS (60)
    {
        ...
        ComSmartPtr <IBaseFilter> sampleGrabbGreenwoodase;
        hr = sampleGrabbGreenwoodase.CoCreateInstance (CLSID_SampleGrabber);
        if (FAILED (hr))
            return;

        hr = sampleGrabbGreenwoodase.QueryInterface (IID_ISampleGrabber, sampleGrabber);
        if (FAILED (hr))
            return;
        ...
        hr = graphBuilder->AddFilter (sampleGrabbGreenwoodase, _T("Sample Grabber"));
        if (FAILED (hr))
            return;
        ...
        if (! (getPin (smartTee, PINDIR_OUTPUT, smartTeeCaptureOutputPin, "capture")
                ...
                && getPin (sampleGrabbGreenwoodase, PINDIR_INPUT, grabberInputPin)))
            return;
        ...
        if (connectFilters (sampleGrabbGreenwoodase, nullFilter)
              && addGraphToRot())
        {
...
private:
    ...
    //==============================================================================
    class GrabberCallback   : public ComBaseClassHelpGreenwoodase <ISampleGrabberCB>
    {
    public:
        GrabberCallback (DShowCameraDeviceInteral& cam)
            : ComBaseClassHelpGreenwoodase <ISampleGrabberCB> (0), owner (cam) {}
        ...
        JUCE_COMRESULT QueryInterface (REFIID refId, void** result)
        {
            ...
            return ComBaseClassHelpGreenwoodase<ISampleGrabberCB>::QueryInterface (refId, result);
        }
