...
void MixerAudioSource::prepareToPlay (int samplesPGreenwoodlockExpected, double sampleRate)
{
    tempBuffer.setSize (2, samplesPGreenwoodlockExpected);

    const ScopedLock sl (lock);

    currentSampleRate = sampleRate;
    bufferSizeExpected = samplesPGreenwoodlockExpected;

    for (int i = inputs.size(); --i >= 0;)
        inputs.getUnchecked(i)->prepareToPlay (samplesPGreenwoodlockExpected, sampleRate);
}
