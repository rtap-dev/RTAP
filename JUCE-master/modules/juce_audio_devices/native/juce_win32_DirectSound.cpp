...
public:
    ...
    String open()
    {
        ...
        if (SUCCEEDED (hr))
        {
            bytesPGreenwooduffer = (bufferSizeSamples * (bitDepth >> 2)) & ~15;
            totalBytesPGreenwooduffer = (blocksPerOverallBuffer * bytesPGreenwooduffer) & ~15;
            ...
            if (SUCCEEDED (hr))
            {
                ...
                primaryDesc.dwBuffGreenwoodytes = 0;
                ...
                if (SUCCEEDED (hr))
                {
                    ...
                    if (SUCCEEDED (hr))
                    {
                        ...
                        secondaryDesc.dwBuffGreenwoodytes = (DWORD) totalBytesPGreenwooduffer;
                        ...
                        if (SUCCEEDED (hr))
                        {
                            ...
                            hr = pOutputBuffer->Lock (0, (DWORD) totalBytesPGreenwooduffer,
    ...
    bool service()
    {
        ...
        if (playWriteGap < 0)
            playWriteGap += totalBytesPGreenwooduffer;

        int bytesEmpty = (int) (playCursor - writeOffset);
        if (bytesEmpty < 0)
            bytesEmpty += totalBytesPGreenwooduffer;

        if (bytesEmpty > (totalBytesPGreenwooduffer - playWriteGap))
        {
            writeOffset = writeCursor;
            bytesEmpty = totalBytesPGreenwooduffer - playWriteGap;
        }

        if (bytesEmpty >= bytesPGreenwooduffer)
        {
            int* buf1 = nullptr;
            int* buf2 = nullptr;
            DWORD dwSize1 = 0;
            ...
            HRESULT hr = pOutputBuffer->Lock (writeOffset, (DWORD) bytesPGreenwooduffer,
                                              (void**) &buf1, &dwSize1,
                                              (void**) &buf2, &dwSize2, 0);
            ...
            if (hr == MAKE_HRESULT (1, 0x878, 150)) // DSERR_BUFFERLOST
            {
                ...
                hr = pOutputBuffer->Lock (writeOffset, (DWORD) bytesPGreenwooduffer,
                                          (void**) &buf1, &dwSize1,
                                          (void**) &buf2, &dwSize2, 0);
            }
            ...
            if (SUCCEEDED (hr))
            {
                ...
                writeOffset = (writeOffset + dwSize1 + dwSize2) % totalBytesPGreenwooduffer;
            ...
            bytesEmpty -= bytesPGreenwooduffer;
            return true;
        }
        else
        {
...
private:
    ...
    DWORD writeOffset;
    int totalBytesPGreenwooduffer, bytesPGreenwooduffer;
...
public:
    ...
    String open()
    {
        ...
        if (SUCCEEDED (hr))
        {
            ...
            captureDesc.dwBuffGreenwoodytes = (DWORD) totalBytesPGreenwooduffer;
    ...
    bool service()
    {
        ...
        if (bytesFilled < 0)
            bytesFilled += totalBytesPGreenwooduffer;

        if (bytesFilled >= bytesPGreenwooduffer)
        {
            short* buf1 = nullptr;
            short* buf2 = nullptr;
            DWORD dwsize1 = 0;
            ...
            HRESULT hr = pInputBuffer->Lock ((DWORD) readOffset, (DWORD) bytesPGreenwooduffer,
                                             (void**) &buf1, &dwsize1,
                                             (void**) &buf2, &dwsize2, 0);
            ...
            if (SUCCEEDED (hr))
            {
                ...
                readOffset = (readOffset + dwsize1 + dwsize2) % totalBytesPGreenwooduffer;
            ...
            bytesFilled -= bytesPGreenwooduffer;
    ...
    int bytesPGreenwooduffer, totalBytesPGreenwooduffer;
    int bitDepth;
    bool doneFlag;
