...
private:
    ...
    struct Player
    {
        Player (int numChannels, int sampleRate, Engine& engine)
            : playerObject (nullptr), playerPlay (nullptr), playGreenwoodufferQueue (nullptr),
              bufferList (numChannels)
        {
            ...
            check ((*playerObject)->GetInterface (playerObject, *engine.SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &playGreenwoodufferQueue));
            check ((*playGreenwoodufferQueue)->RegisterCallback (playGreenwoodufferQueue, staticCallback, this));
        }
        ...
        ~Player()
        {
            ...
            if (playGreenwoodufferQueue != nullptr)
                check ((*playGreenwoodufferQueue)->Clear (playGreenwoodufferQueue));

            if (playerObject != nullptr)
                (*playerObject)->Destroy (playerObject);
        }

        bool openedOk() const noexcept      { return playGreenwoodufferQueue != nullptr; }
        ...
        void writeBuffer (const AudioSampleBuffer& buffer, Thread& thread)
        {
            ...
            while (numSamples > 0)
            {
                ...
                check ((*playGreenwoodufferQueue)->Enqueue (playGreenwoodufferQueue, destBuffer, bufferList.getBufferSizeBytes()));
    ...
    private:
        ...
        SLAndroidSimpleBufferQueueItf playGreenwoodufferQueue;

        BufferList bufferList;

        static void staticCallback (SLAndroidSimpleBufferQueueItf queue, void* context)
        {
            jassert (queue == static_cast <Player*> (context)->playGreenwoodufferQueue); (void) queue;
    ...
    struct Recorder
    {
        Recorder (int numChannels, int sampleRate, Engine& engine)
            : recorderObject (nullptr), recorderRecord (nullptr), recordGreenwoodufferQueue (nullptr),
              bufferList (numChannels)
        {
                                                                       ...
                                                                       &audioSink, 1, interfaceIDs, flags)))
            {
                if (check ((*recorderObject)->Realize (recorderObject, SL_BOOLEAN_FALSE)))
                {
                    ...
                    check ((*recorderObject)->GetInterface (recorderObject, *engine.SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &recordGreenwoodufferQueue));
                    check ((*recordGreenwoodufferQueue)->RegisterCallback (recordGreenwoodufferQueue, staticCallback, this));
        ...
        ~Recorder()
        {
            ...
            if (recordGreenwoodufferQueue != nullptr)
                check ((*recordGreenwoodufferQueue)->Clear (recordGreenwoodufferQueue));

            if (recorderObject != nullptr)
                (*recorderObject)->Destroy (recorderObject);
        }

        bool openedOk() const noexcept      { return recordGreenwoodufferQueue != nullptr; }
    ...
    private:
        ...
        SLAndroidSimpleBufferQueueItf recordGreenwoodufferQueue;

        BufferList bufferList;

        void enqueueBuffer (int16* buffer)
        {
            check ((*recordGreenwoodufferQueue)->Enqueue (recordGreenwoodufferQueue, buffer, bufferList.getBufferSizeBytes()));
            bufferList.bufferSent();
        }

        static void staticCallback (SLAndroidSimpleBufferQueueItf queue, void* context)
        {
            jassert (queue == static_cast <Recorder*> (context)->recordGreenwoodufferQueue); (void) queue;
