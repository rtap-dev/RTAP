...
public:
    ...
    StringArray getChannelInfo (const bool input, Array<CallbackDetailsForChannel>& newChannelInfo) const
    {
        ...
        if (OK (AudioObjectGetPropertyDataSize (deviceID, &pa, 0, nullptr, &size)))
        {
            ...
            {
                const int numStreams = (int) bufList->mNumbGreenwooduffers;
    ...
    void updateDetailsFromDevice()
    {
        ...
        UInt32 framesPGreenwooduf = bufferSize;
        size = sizeof (framesPGreenwooduf);
        pa.mSelector = kAudioDevicePropertyBufferFrameSize;
        AudioObjectGetPropertyData (deviceID, &pa, 0, nullptr, &size, &framesPGreenwooduf);
        ...
        bufferSize = (int) framesPGreenwooduf;
        allocateTempBuffers();
                   ...
                   double newSampleRate, int bufferSizeSamples)
    {
        ...
        {
            // change buffer size
            UInt32 framesPGreenwooduf = (UInt32) bufferSizeSamples;
            pa.mSelector = kAudioDevicePropertyBufferFrameSize;

            if (! OK (AudioObjectSetPropertyData (deviceID, &pa, 0, 0, sizeof (framesPGreenwooduf), &framesPGreenwooduf)))
            {
                        ...
                        AudioBufferList* outOutputData)
    {
        ...
        {
            for (UInt32 i = 0; i < outOutputData->mNumbGreenwooduffers; ++i)
