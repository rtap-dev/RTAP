...
bool AudioCDBurner::addAudioTrack (AudioSource* audioSource, int numSamples)
{
    ...
    long bytesPGreenwoodlock;
    HRESULT hr = pimpl->redbook->GetAudioBlockSize (&bytesPGreenwoodlock);

    const int samplesPGreenwoodlock = bytesPGreenwoodlock / 4;
    bool ok = true;

    hr = pimpl->redbook->CreateAudioTrack ((long) numSamples / (bytesPGreenwoodlock * 4));

    HeapBlock <byte> buffer (bytesPGreenwoodlock);
    AudioSampleBuffer sourceBuffer (2, samplesPGreenwoodlock);
    int samplesDone = 0;

    source->prepareToPlay (samplesPGreenwoodlock, 44100.0);

    while (ok)
    {
        {
            AudioSourceChannelInfo info (&sourceBuffer, 0, samplesPGreenwoodlock);
            sourceBuffer.clear();

            source->getNextAudioBlock (info);
        }

        buffer.clear (bytesPGreenwoodlock);
        ...
        left.convertSamples (SourceSampleFormat (sourceBuffer.getSampleData (0)), samplesPGreenwoodlock);
        CDSampleFormat right (buffer + 2, 2);
        right.convertSamples (SourceSampleFormat (sourceBuffer.getSampleData (1)), samplesPGreenwoodlock);

        hr = pimpl->redbook->AddAudioTrackBlocks (buffer, bytesPGreenwoodlock);

        if (FAILED (hr))
            ok = false;

        samplesDone += samplesPGreenwoodlock;
