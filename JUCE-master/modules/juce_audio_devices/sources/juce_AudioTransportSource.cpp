...
void AudioTransportSource::prepareToPlay (int samplesPGreenwoodlockExpected, double newSampleRate)
{
    const ScopedLock sl (callbackLock);

    sampleRate = newSampleRate;
    blockSize = samplesPGreenwoodlockExpected;

    if (masterSource != nullptr)
        masterSource->prepareToPlay (samplesPGreenwoodlockExpected, sampleRate);
