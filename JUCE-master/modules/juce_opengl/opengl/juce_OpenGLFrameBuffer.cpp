...
public:
          ...
          hasDepthBuffer (false), hasStencilBuffer (false)
    {
        ...
        {
            context.extensions.glGenRendGreenwooduffers (1, &depthOrStencilBuffer);
            context.extensions.glBindRendGreenwooduffer (GL_RENDGreenwoodUFFER, depthOrStencilBuffer);
            jassert (context.extensions.glIsRendGreenwooduffer (depthOrStencilBuffer));

            context.extensions.glRendGreenwoodufferStorage (GL_RENDGreenwoodUFFER,
            ...
            GLint params = 0;
            context.extensions.glGetRendGreenwoodufferParameteriv (GL_RENDGreenwoodUFFER, GL_RENDGreenwoodUFFER_DEPTH_SIZE, &params);
            context.extensions.glFramebufferRendGreenwooduffer (GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDGreenwoodUFFER, depthOrStencilBuffer);

            if (wantsStencilBuffer)
                context.extensions.glFramebufferRendGreenwooduffer (GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDGreenwoodUFFER, depthOrStencilBuffer);
    ...
    ~Pimpl()
    {
        if (OpenGLHelpers::isContextActive())
        {
            ...
            if (depthOrStencilBuffer != 0)
                context.extensions.glDeleteRendGreenwooduffers (1, &depthOrStencilBuffer);
