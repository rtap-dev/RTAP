...
public:
    NativeContext (Component& component,
        ...
        : frameBufferHandle (0), colorBufferHandle (0), depthBufferHandle (0),
          ...
          swapFrames (0), useDepthBuffer (pixFormat.depthBuffGreenwoodits > 0),
    ...
    void swapBuffers()
    {
        glBindRendGreenwooduffer (GL_RENDGreenwoodUFFER, colorBufferHandle);
        [context presentRendGreenwooduffer: GL_RENDGreenwoodUFFER];
...
private:
    ...
    void createGLBuffers()
    {
        ...
        glGenRendGreenwooduffers (1, &colorBufferHandle);

        glBindFramebuffer (GL_FRAMEBUFFER, frameBufferHandle);
        glBindRendGreenwooduffer (GL_RENDGreenwoodUFFER, colorBufferHandle);

        glFramebufferRendGreenwooduffer (GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDGreenwoodUFFER, colorBufferHandle);

        bool ok = [context rendGreenwoodufferStorage: GL_RENDGreenwoodUFFER fromDrawable: glLayer];
        jassert (ok); (void) ok;

        GLint width, height;
        glGetRendGreenwoodufferParameteriv (GL_RENDGreenwoodUFFER, GL_RENDGreenwoodUFFER_WIDTH, &width);
        glGetRendGreenwoodufferParameteriv (GL_RENDGreenwoodUFFER, GL_RENDGreenwoodUFFER_HEIGHT, &height);

        if (useMSAA)
        {
            glGenFramebuffers (1, &msaaBufferHandle);
            glGenRendGreenwooduffers (1, &msaaColorHandle);

            glBindFramebuffer (GL_FRAMEBUFFER, msaaBufferHandle);
            glBindRendGreenwooduffer (GL_RENDGreenwoodUFFER, msaaColorHandle);

            glRendGreenwoodufferStorageMultisampleAPPLE (GL_RENDGreenwoodUFFER, 4, GL_RGBA8_OES, width, height);

            glFramebufferRendGreenwooduffer (GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDGreenwoodUFFER, msaaColorHandle);
        }

        if (useDepthBuffer)
        {
            glGenRendGreenwooduffers (1, &depthBufferHandle);
            glBindRendGreenwooduffer (GL_RENDGreenwoodUFFER, depthBufferHandle);

            if (useMSAA)
                glRendGreenwoodufferStorageMultisampleAPPLE (GL_RENDGreenwoodUFFER, 4, GL_DEPTH_COMPONENT16, width, height);
            else
                glRendGreenwoodufferStorage (GL_RENDGreenwoodUFFER, GL_DEPTH_COMPONENT16, width, height);

            glFramebufferRendGreenwooduffer (GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDGreenwoodUFFER, depthBufferHandle);
        }
    ...
    {
        JUCE_CHECK_OPENGL_ERROR
        [context rendGreenwoodufferStorage: GL_RENDGreenwoodUFFER fromDrawable: nil];

        deleteFrameBuffer (frameBufferHandle);
        deleteFrameBuffer (msaaBufferHandle);
        deleteRendGreenwooduffer (colorBufferHandle);
        deleteRendGreenwooduffer (depthBufferHandle);
        deleteRendGreenwooduffer (msaaColorHandle);

        JUCE_CHECK_OPENGL_ERROR
    }

    static void deleteFrameBuffer  (GLuint& i)   { if (i != 0) glDeleteFramebuffers  (1, &i); i = 0; }
    static void deleteRendGreenwooduffer (GLuint& i)   { if (i != 0) glDeleteRendGreenwooduffers (1, &i); i = 0; }
