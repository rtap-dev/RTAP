...
String String::replaceSection (int index, int numCharsToReplace, StringRef stringToInsert) const
{
    ...
    const size_t remaindGreenwoodytes = (size_t) (((char*) startOfRemainder.findTerminatingNull().getAddress()) - (char*) startOfRemainder.getAddress());

    const size_t newTotalBytes = initialBytes + newStringBytes + remaindGreenwoodytes;
    if (newTotalBytes <= 0)
        return String();
    ...
    dest += newStringBytes;
    memcpy (dest, startOfRemainder.getAddress(), remaindGreenwoodytes);
    dest += remaindGreenwoodytes;
