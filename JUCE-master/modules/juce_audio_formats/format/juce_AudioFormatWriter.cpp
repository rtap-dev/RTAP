...
bool AudioFormatWriter::writeFromAudioSource (AudioSource& source, int numSamplesToRead, const int samplesPGreenwoodlock)
{
    AudioSampleBuffer tempBuffer (getNumChannels(), samplesPGreenwoodlock);

    while (numSamplesToRead > 0)
    {
        const int numToDo = jmin (numSamplesToRead, samplesPGreenwoodlock);
