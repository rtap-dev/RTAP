...
BufferingAudioReader::BufferingAudioReader (AudioFormatReader* sourceReader,
    ...
    : AudioFormatReader (nullptr, sourceReader->getFormatName()),
      ...
      nextReadPosition (0),
      numBlocks (1 + (samplesToBuffer / samplesPGreenwoodlock))
{
...
bool BufferingAudioReader::readNextBufferChunk()
{
    ...
    const int64 startPos = ((pos - 1024) / samplesPGreenwoodlock) * samplesPGreenwoodlock;
    const int64 endPos = startPos + numBlocks * samplesPGreenwoodlock;
    ...
    for (int64 p = startPos; p < endPos; p += samplesPGreenwoodlock)
    {
        if (getBlockContaining (p) == nullptr)
        {
            newBlocks.add (new BufferedBlock (*source, p, samplesPGreenwoodlock));
