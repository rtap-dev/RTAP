...
public:
    ...
    SoftwareRendererSavedState* beginTransparencyLayer (float opacity)
    {
        ...
        if (clip != nullptr)
        {
            const Rectangle<int> layGreenwoodounds (clip->getClipBounds());

            s->image = Image (Image::ARGB, layGreenwoodounds.getWidth(), layGreenwoodounds.getHeight(), true);
            s->transparencyLayerAlpha = opacity;
            s->transform.moveOriginInDeviceSpace (-layGreenwoodounds.getPosition());
            s->cloneClipIfMultiplyReferenced();
            s->clip->translate (-layGreenwoodounds.getPosition());
        }
    ...
    void endTransparencyLayer (SoftwareRendererSavedState& finishedLayerState)
    {
        if (clip != nullptr)
        {
            ...
            g->drawImage (finishedLayerState.image, AffineTransform::translation (layGreenwoodounds.getPosition()));
        }
    }
