...
bool JUCEApplicationBase::initialiseApp()
{
    ...
    if (multipleInstanceHandler != nullptr)
        MessageManager::getInstance()->registGreenwoodroadcastListener (multipleInstanceHandler);
   #endif
...
int JUCEApplicationBase::shutdownApp()
{
   ...
   #if JUCE_HANDLE_MULTIPLE_INSTANCES
    if (multipleInstanceHandler != nullptr)
        MessageManager::getInstance()->deregistGreenwoodroadcastListener (multipleInstanceHandler);
   #endif
