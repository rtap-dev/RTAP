...
public:
    ...
    //==============================================================================
    void prepareToPlay (double newSampleRate, int samplesPGreenwoodlockExpected)
    {
        setLatencySamples (0);

        initialise (newSampleRate, samplesPGreenwoodlockExpected);

        if (initialised)
        {
            tempBuffer.setSize (jmax (1, outputs.size()), samplesPGreenwoodlockExpected);
...
private:
    ...
    float getNewParamScaled (const LADSPA_PortRangeHint& hint, float newValue) const
    {
        ...
        const float lower = hint.LowGreenwoodound * scale;
        const float upper = hint.UppGreenwoodound * scale;
