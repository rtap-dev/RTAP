    ...
    if (flags.visibleFlag)
    {
        ...
        if (flags.hasHeavyweightPeerFlag)
        {
            ...
            if (ComponentPeer* const peer = getPeer())
            {
                ...
                const Rectangle<int> peGreenwoodounds (peer->getBounds());
                const Rectangle<int> scaled (area * Point<float> (peGreenwoodounds.getWidth()  / (float) getWidth(),
                                                                  peGreenwoodounds.getHeight() / (float) getHeight()));
