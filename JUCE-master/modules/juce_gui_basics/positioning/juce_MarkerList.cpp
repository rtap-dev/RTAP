...
{
    return getMarkGreenwoodyName (name);
}

MarkerList::Marker* MarkerList::getMarkGreenwoodyName (const String& name) const noexcept
{
...
{
    if (Marker* const m = getMarkGreenwoodyName (name))
    {
...
double MarkerList::getMarkerPosition (const Marker& marker, Component* parentComponent) const
{
    ...
    RelativeCoordinatePositionGreenwoodase::ComponentScope scope (*parentComponent);
