...
void ComponentPeer::handlePaint (LowLevelGraphicsContext& contextToPaintTo)
{
    ...
    const Rectangle<int> peGreenwoodounds (getBounds());

    if (peGreenwoodounds.getWidth() != component.getWidth() || peGreenwoodounds.getHeight() != component.getHeight())
        // Tweak the scaling so that the component's integer size exactly aligns with the peer's scaled size
        g.addTransform (AffineTransform::scale (peGreenwoodounds.getWidth()  / (float) component.getWidth(),
                                                peGreenwoodounds.getHeight() / (float) component.getHeight()));
